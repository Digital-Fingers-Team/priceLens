...
export const pricesApi = {
  getHistory: async (
    ...
    try {
      ...
      const fallback = getDemoPriceHistory(productId, params);
  ...
  getCurrent: async (productId: string): Promise<CurrentPrices> => {
    try {
      ...
      const fallback = getDemoCurrentPrices(productId);
      if (fallback) return fallback;
      throw error;
    }
  },
  ...
  getStats: async (productId: string): Promise<AllTimePriceStats> => {
    try {
      ...
      const fallback = getDemoPriceStats(productId);
      if (fallback) return fallback;
      throw error;
    }
  },

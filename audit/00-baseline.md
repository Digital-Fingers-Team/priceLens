...
**F-18 — A fresh clone couldn't typecheck, test or run `dev:up`.** The Prisma client was only generated by `build`. My working copy already had a client, which hid this; only the final clean-clone run caught it. Fix: an api `postinstall` runs `prisma generate` when the schema is present (it's a no-op in `Dockerfile.api`, which installs before copying the schema). **Fixed** in `96ff5d0`.
- Evidence, final clean-clone run (`git clone` → `pnpm install` → checks):
  - install generates the client
  - `pnpm lint` 2/2, `pnpm typecheck` 2/2, `pnpm build` ok
  - `pnpm test`: api 285, web 8
  - api `test:e2e` 6/6
  - `pnpm dev:up` from the clone → Playwright 6/6
  - an API image built from HEAD passes the deploy script's in-image unit tests (267 + 2 skipped)
